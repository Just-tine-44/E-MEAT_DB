...
--
-- Host: 127.0.0.1
-- Generation Time: May 04, 2025 at 07:15 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Add_Stock_To_MEAT_PART` (IN `p_meat_part_id` INT, IN `p_additional_qty` DECIMAL(10,2), IN `p_unit_price` DECIMAL(15,2))   BEGIN
    -- Update the quantity available (add to current value) and unit price
    UPDATE MEAT_PART
    SET QTY_AVAILABLE = QTY_AVAILABLE + p_additional_qty,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCustomerPurchases` (IN `p_start_date` DATETIME, IN `p_end_date` DATETIME, IN `p_limit` INT)   BEGIN
    SELECT 
        CONCAT(u.USER_FNAME, ' ', u.USER_LNAME) as customer_name,
        u.USERNAME as email,
        COUNT(DISTINCT o.ORDERS_ID) as order_count,
        SUM(od.QTY) as items_purchased,
        MAX(o.ORDERS_DATE) as last_purchase,
        SUM(o.TOTAL_AMOUNT) as total_spent
    FROM APP_USER u
    JOIN ORDERS o ON u.APP_USER_ID = o.APP_USER_ID
    JOIN ORDERS_DETAIL od ON o.ORDERS_ID = od.ORDERS_ID
    WHERE o.ORDERS_DATE BETWEEN p_start_date AND p_end_date
    GROUP BY u.APP_USER_ID
    ORDER BY total_spent DESC
    LIMIT p_limit;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDailySales` (IN `p_start_date` DATETIME, IN `p_end_date` DATETIME)   BEGIN
    SELECT 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetMeatPurchaseDetailedByDate` (IN `start_date` DATETIME, IN `end_date` DATETIME)   BEGIN
    SELECT 
        CONCAT(u.USER_FNAME, ' ', u.USER_LNAME) AS customer_name,
    ...
    JOIN 
        APP_USER u ON o.APP_USER_ID = u.APP_USER_ID
    WHERE
        o.ORDERS_DATE BETWEEN start_date AND end_date
    ORDER BY 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetSales` (IN `p_start_date` DATETIME, IN `p_end_date` DATETIME)   BEGIN
    SELECT 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetSalesOverviewByDate` (IN `start_date` DATETIME, IN `end_date` DATETIME)   BEGIN
    -- Combined query returning a single row with all metrics
    SELECT
        (SELECT COALESCE(SUM(TOTAL_AMOUNT), 0)
         FROM ORDERS
         WHERE STAT_ID IS NOT NULL
         AND ORDERS_DATE BETWEEN start_date AND end_date) AS total_sales_period,
         
        (SELECT COALESCE(SUM(TOTAL_AMOUNT), 0)
         FROM ORDERS
         WHERE STAT_ID IS NOT NULL
         AND ORDERS_DATE >= NOW() - INTERVAL 1 DAY) AS total_sales_last_1_day,
         
        (SELECT COALESCE(SUM(TOTAL_AMOUNT), 0)
         FROM ORDERS
         WHERE STAT_ID IS NOT NULL
         AND YEARWEEK(ORDERS_DATE, 1) = YEARWEEK(CURDATE(), 1)) AS total_sales_this_week,
         
        (SELECT COALESCE(SUM(TOTAL_AMOUNT), 0)
         FROM ORDERS
         WHERE STAT_ID IS NOT NULL
         AND YEAR(ORDERS_DATE) = YEAR(CURDATE())
         AND MONTH(ORDERS_DATE) = MONTH(CURDATE())) AS total_sales_this_month;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetTopProducts` (IN `p_start_date` DATETIME, IN `p_end_date` DATETIME, IN `p_limit` INT)   BEGIN
    SELECT 
        mp.MEAT_PART_NAME as product_name,
        IFNULL(SUM(od.LINE_TOTAL), 0) as total_sales,
        SUM(od.QTY) as quantity_sold,
        od.UNIT_OF_MEASURE as unit_measure
    FROM ORDERS_DETAIL od
    JOIN ORDERS o ON o.ORDERS_ID = od.ORDERS_ID
    JOIN MEAT_PART mp ON od.MEAT_PART_ID = mp.MEAT_PART_ID
    WHERE o.ORDERS_DATE BETWEEN p_start_date AND p_end_date
    GROUP BY mp.MEAT_PART_ID, od.UNIT_OF_MEASURE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_update_rider_assigned` (IN `p_rider_id` INT, IN `p_order_id` INT, OUT `p_success` BOOLEAN, OUT `p_message` VARCHAR(255))   BEGIN
    DECLARE rider_exists INT;
    ...
    SELECT COUNT(*) INTO rider_exists FROM RIDER WHERE rider_id = p_rider_id;
    
    -- Check if order exists
    SELECT COUNT(*) INTO order_exists FROM ORDERS WHERE ORDERS_ID = p_order_id;
    
    -- Validate inputs
    IF rider_exists = 0 THEN
        SET p_message = 'Rider not found';
    ELSEIF order_exists = 0 THEN
        SET p_message = 'Order not found';
    ELSE
        -- Update the order with the rider ID
        UPDATE ORDERS
        ...
        IF ROW_COUNT() > 0 THEN
            SET p_success = TRUE;
            SET p_message = 'Rider assigned successfully';
        ELSE
            SET p_message = 'Failed to assign rider';
        END IF;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateRider` (IN `p_rider_id` INT, IN `p_rider_name` VARCHAR(50), IN `p_contact` VARCHAR(20), OUT `p_success` BOOLEAN, OUT `p_message` VARCHAR(255))   BEGIN
    DECLARE rider_exists INT DEFAULT 0;
    ...
    ELSE
        ...
        ELSE
            SET p_success = TRUE; -- Still consider it a success
            SET p_message = 'No changes made';
        END IF;
    END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Update_MEAT_PART_Price` (IN `p_meat_part_id` INT, IN `p_unit_price` DECIMAL(15,2))   BEGIN
    ...
    IF current_price <> p_unit_price THEN
        UPDATE MEAT_PART
        SET UNIT_PRICE = p_unit_price,
            LAST_UPDATED = CURRENT_TIMESTAMP()
        WHERE MEAT_PART_ID = p_meat_part_id;
        
        SELECT 'Price updated successfully' AS message, 1 AS success;
    ELSE
        SELECT 'No changes made to price' AS message, 0 AS success;
    END IF;
END$$
...
INSERT INTO `meat_part` (`MEAT_PART_ID`, `APP_USER_ID`, `MEAT_CATEGORY_ID`, `MEAT_PART_NAME`, `MEAT_PART_PHOTO`, `QTY_AVAILABLE`, `UNIT_OF_MEASURE`, `UNIT_PRICE`, `LAST_UPDATED`) VALUES
(1, 2, 2, 'Pork Belly', 'Porks Belly.png', 208.30, 'kg', 157.00, '2025-05-03 16:36:28');
...
(1, 3, 1, 1, 5, '2025-04-30 09:42:38', 287.00, '2025-05-03 17:21:29', 2),
(2, 1, 1, 1, 5, '2025-05-04 00:36:28', 395.40, '2025-05-04 00:20:37', 2);
...
(3, 2, 1, 2.00, 'KG', 157.00, 314.00),
(4, 2, 1, 200.00, 'G', 157.00, 31.40);
